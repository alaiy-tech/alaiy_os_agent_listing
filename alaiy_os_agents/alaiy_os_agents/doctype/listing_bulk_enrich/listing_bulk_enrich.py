...
fan-out over Frappe workers lives in ``agents/listing/bulk.py``, which the actions
below hand off to.

One channel per batch. The field spec, the validation rules and the image step all
come from the channel's adapter, so a batch spanning two channels would have no
single answer for any of them — and the skip-already-enriched check would not know
which enriched doctype to look in.
"""
...
from alaiy_os_agents.agents.listing.bulk import DEFAULT_BATCH_SIZE, enqueue_chunks
...
class ListingBulkEnrich(Document):
	def validate(self):
		self._dedupe_items()
